...
def run_experiment(experiment_id: int, train_dir: str, val_dir: str, normalize, best_acc=0, early_stop_counter=0):
    ...
    experiment_signature = model_name + " ID=" + str(experiment_id) + " lr=" + str(learning_rate) + " reg=" + str(
            weight_decay) + " bs=" + str(batch_size)
...
if __name__ == "__main__":
    ...
    for i in range(number_of_experiments):
        ...
        if enable_wandb:
            # provider.plot_confusion_matrix_and_save(cm_4class, ["Mayo 0", "Mayo 1", "Mayo 2", "Mayo 3"],
            #                                         os.path.join(wandb.run.dir, "cm_4class.png"),
            #                                         "confusion matrix (classification) - " + model_name, normalize=True)
            # provider.plot_confusion_matrix_and_save(cm_remission, ["Remission", "Non Remission"],
            #                                         os.path.join(wandb.run.dir, "cm_remission.png"),
            #                                         "confusion matrix (classification) - " + model_name,
            #                                         normalize=True)
